...
During my time at SICS's NES group (Networked Embedded Systems) I was glad to find that its members not only possessed great expertise of their field, but also a willingness to share it with me. I would like to expressly thank Simon Duquennoy for his encouragement when encumbered with technical difficulties in the djungle of Contiki's echo system of development tools, for sharing his thoughtful insights of the problems of the field of IoT during the group's weekly meetings, all while still being a good listener. Continuing, Niclas Finne is a man with great technical insight of every nook and cranny of the Contiki system, including knowledge of obscure compilator and emulator bugs. I owe him a great deal in helping me out with difficult problems that would have taken me a prohibitive amount of time to solve on my own. Gratitude is also owed to Christian Rohner, Thiemo Voigt and Nicolas Tsiftes who (in that order) reviewed my report, who helped me to get started and offered help, and the latter who as a colleague gave me his own excellent report as a reference and offered to review and comment upon mine. Thank you very much! Finally, I would like to thank Navid Hassanzadeh and Beshr Al Nahas for great company in the office and intellectually stimulating discussions during our common commute. Thank you all!

\newpage

\section*{List of Acronyms}
\begin{acronym}
   \acro{AES}{The Advanced Encryption Standard is a standard established by NIST}
   \acro{AES-CBC}{AES Cipher Block Chaining, a block cipher}
   \acro{AES-CTR}{AES Counter, a stream cipher}
   \acro{AES-XCBC}{AES-CBC, but for authentication}
   \acro{AH}{Authentication Header}
   \acro{API}{Application Programming Interface}
   \acro{BSS}{Segment of the binary used for statically allocated variables}
   \acro{DH}{Diffie-Hellman, a method of cryptographic key exchange named after their inventors}
   ...
   \acro{ECP group}{Elliptic Curve Groups modulo a prime is a set of ECC parameters}
   \acro{ESP}{Encapsulated Security Payload}
   \acro{HDR}{Common abbreviation of Header}
   \acro{HMAC}{A keyed-Hash Message Authentication Code is an hash-algorithm incorporating a key for use in authentication purposes}
   \acro{ICMP}{Internet Control Message Protocol is a protocol probably most commonly known for its use by UNIX's `ping' utility}
   \acro{ICV}{Integrity Check Value}
   \acro{IEEE}{Institute of Electrical and Electronics Engineer}
   \acro{IETF}{Internet Engineer Task Force is the co-ordinating body of Internet standards}
   \acro{IKEv2}{Internet Key Exchange version 2}
   \acro{IP}{Internet Protocol}
   \acro{ISM}{Industrial, Scientific and Medical radio band}
   \acro{IoT}{Internet of Things}
   \acro{KEi}{IKE Key Exchange payload, initiator's public key}
   \acro{KEr}{IKE Key Exchange payload, responder's public key}
   \acro{MAC}{Message Authentication Code}
   \acro{MAC}{Media Access Control, sublayer of Data Link layer}
   \acro{MTU}{Maximum Transmission Unit}
   \acro{NAT}{Network Address Translation is commonly used for connection a private IP network to the Internet}
   \acro{Nr}{IKE Nonce payload, initiator's}
   \acro{Nr}{IKE Nonce payload, responder's}
   \acro{OS}{Operating System}
   \acro{PAD}{Peer Authorization Database}
   \acro{PAN}{Personal Area Network is a short range network (with approximately 10 m radius)}
   \acro{PFP}{Populate From Packet}
   \acro{PKCS}{Public-Key Cryptography Standards}
   \acro{PRF}{Pseudo-Random Function}
   \acro{PSTN}{Public Switched Telephone Network, the ordinary phone network}
   \acro{QoS}{Quality of Service}
   \acro{RAM}{Random Access Memory}
   \acro{RFC}{Request For Comment, a class of IETF standards}
   \acro{ROM}{Read-Only Memory}
   \acro{RPL}{Routing protocol commonly used for IoT, pronounced `ripple'}
   \acro{RSA}{Common public-key cryptosystem}
   \acro{SA}{Security Association}
   \acro{SAD}{Security Association Database}
   \acro{SHA1}{}
   \acro{SICS}{Swedish Institute of Computer Science}
   \acro{SK}{IKE Payload: Encapsulation of payloads to be protected}
   \acro{SLOC}{Source Lines Of Code}
   \acro{SPD}{Security Policy Database}
   \acro{SPI}{Security Parameter Index}
   \acro{TCP}{Transmission Control Protocol, an IP Transport Layer protocol}
   \acro{TLS}{Transport Layer Security, an IETF protocol}
   \acro{TOS}{Type Of Service}
   \acro{TSi}{IKE payload: Traffic Selector, initiator's side}
   \acro{TSr}{IKE payload: Traffic Selector, responder's side}
   \acro{TripleDES}{Triple Data Encryption Standard from NIST}
   \acro{UDP}{User Datagram Protocol}
   \acro{X.509}{ITU-T (International Telephony Union) standard for public key infrastructure (PKI)}
\end{acronym}
...
Multi\-tasking (concurrency) is provided in Contiki by the \emph{Proto\-threads} mechanism\cite{dunkels2005protothreads}. Before explaining its inner workings, I will provide the reader with a short review of the problems associated with multitasking in resource constrained systems.
...
\section{IPsec}
At the project's inception, there was already a rudimentary implementation of IPsec in place (described in \cite{raza11securing}). It had the ability to receive as well as transmit AH and ESP headers, but could only handle one set of security parameters, effectively limiting the host to one SA that was used for duplex traffic. This implementation served as a proof-of-concept of the basics of IPsec on Contiki, as well as an IPsec header compression scheme in the underlying MAC-layer. Having stated this, the lack of support for multiple SAs and the inability to negotiate new ones made it difficult to fit it into a real world network environment where communication patterns are subject to constant change and keys need to be renewed as they age and lose their security properties. % This is where I talk about the benefits of multiple SAs
...
Briefly, the reason that asymmetric encryption is so resource intensive is that it's centered around operations on very large numbers. The operations are very simple (e.g. $A = g^{a} \mod p$), but in the case of the popular RSA scheme, the numbers $a$ and $A$, which forms the so called public and private key, respectively, can be so large that they require 3 072 bits to represent. A DH exchange with such a long key is most often not feasible in an IoT context.
...
\chapter{Evaluation}
\label{cha:eval}
The software was evaluated using the Cooja application (as stated in \prettyref{sec:finaldev}). As Cooja is a fully featured simulation environment, not only could the software be tested on emulated hardware, a complete simulation of the radio environment using several motes can be performed as well. Although the radio / network simulation was used to make the evaluation (as described below) more realistic, my main focus was the fidelity of the hardware emulation.

The CPU and some peripherals (most notably the CC2540 radio) is emulated on the instruction-level with an efficiency that allowed me to run two motes simultaneously with real-time execution speeds. This allows Cooja (most importantly) to accurately model memory, interrupts and radio - MCU interaction. The result is a simulation that closely mimics the behavior of the real hardware (including its network timing behavior), with the exceptions of possible emulator bugs. Thankfully, my IPsec / IKEv2 implementation is not dependent upon any special features of the hardware, but only concerns itself with IP, and is thus not likely to be affected by any of the aforementioned bugs.
...
\begin{itemize}
   ...
   \item SAD, with mechanism for SA expiration % Multiple SAs
...
\begin{itemize}
   ...
   \item Multiple child SAs per IKE SA % Multiple SAs
\end{itemize}
...
Wismote's memory limits                & 128 kB                   & 16 kB                       
\end{tabular}
...
\label{tab:eval-heap}
\begin{tabular}{c|l}
Exchange                      & Maximum amount of allocated heap           \\ \hline
...
Wismote's RAM limit     & N/A          & N/A                   & N/A                   & 16 kB
\end{tabular}
...
Although the reader might find this criticism harsh, I believe it's warranted. Though, I would like to you to keep in mind that this criticism concerns the standards as a whole, and not their parts. For example, I regard the ESP protocol to be a rather elegant solution to the problem of IP security, and that it possibly could be re-used in another security standard.
...
\label{sec:fw}
If this implementation is to be used in a production environment it needs to be improved both in terms of quality and in terms of features. The later is important for interoperability.

Still, even though I have shown that IPsec and IKEv2 probably can be used in real-world applications, the benefits of this are dubious for reasons described previously in this chapter. Therefore I recommend that future research be turned towards other standards, unless IPsec / IKEv2 is required for integration with an existing network.

Furthermore, this work also revealed a problem that is in common to all security protocol implementations on Contiki - the lack of a source of entropy for initialization of security parameters e.g. private keys. The implementation of such a system service might serve as a suitable subject for a bachelor thesis.
...
Can IPsec and IKEv2 be implemented within the current hardware boundaries while still being interoperable with other Internet hosts?
\end{quotation}
My conclusion, based on the results presented in this thesis, is as follows:
\begin{quotation}
Yes, IPsec and IKEv2 can be implemented in such a manner that interoperability with the vast majority of Internet hosts is achievable. However, other standards for secure communication (such as TLS) are probably more suitable for the IoT.
\end{quotation}
