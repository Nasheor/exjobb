...
The author found the body of standard documents to be large, complex and describe many features. It was unclear what effects a removal or alteration of a behavior could have on the implementation. Therefore the following principle was decided upon:
...
\section{IPsec}
At the project's inception, there was already a rudimentary implementation of IPsec in place (described in \cite{raza12secure}). It had the ability to receive as well as transmit AH and ESP headers, but could only handle one set of security parameters, effectively limiting the host to one SA that was used for duplex traffic. This implementation served as a proof-of-concept of the basics of IPsec on Contiki, as well as an IPsec header compression scheme in the underlying MAC-layer. Having stated this, the lack of support for multiple SAs and the inability to negotiate new ones made it difficult to fit it into a real world network environment where communication patterns are subject to constant change.

In this section, the author will begin by describing the IPsec implementation's design specification, its rationale, and most importantly its RFC-compliance. It will then continue with a review of how the technical problems have been solved.
...
%\paragraph{The AH protocol and associated extension header will not be implemented} as it is made redundant . 



\subsection{The AH and ESP protocol} % Ska vi verkligen ha den här rubriken här?
The AH and ESP protocols are described in RFC 4302 and 4303, respectively.
Must support popular encryption types.

% Explain why tunnel mode is not implemented

\subsection{The AH protocol}
AH not required (explain! get stuff from e-mails). ESP is. Must be able to handle headers in any place
...
1. Overview of uIP stack code structure. Doxygen code graphs are probably not available. Make something on your own instead. Show where we insert our hooks.
2. Describe in order to appearance starting with incoming traffic: ESP decoding, databases. Outoing: policy checking with databases, encoding
