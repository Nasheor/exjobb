...
\end{tabular}
\caption{Size of the stack at different times during the key exchange.}
\end{figure}
...
\end{tabular}
\caption{Size of the heap at different times during the key exchange.}
\end{figure}
...
The evaluation of the image's ROM and RAM usage was straightforward and the numbers should be correct, caveat the following: the test application's (the UDP message `bouncer') was included in all images of table \cite{tab:rom-ram-eval}; no status, nor any error message strings were included in the binary, making in-field diagnostics difficult.

Stack and heap measurement figures should be correct to the byte by virtue of the evaluation techniques described in \cite{cha:eval}.
...
Diskutera mina resultat: Styrkor och svagheter i vad gäller undersökningsmetodiken såväl som resultatet.
