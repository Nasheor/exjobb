...
The AH protocol's header (\emph{Authentication Header}) protocol guarantees the \emph{data integrity} and the \emph{authentication} (origin) of the IP packet which it is applied to. This is achieved by applying a \emph{Message Authentication Code} (MAC) to the packet's content and to the header's immutable fields using a key privy to the two parties.
...
The ESP protocol header, provides \emph{confidentiality} in addition to \emph{data integrity} and \emph{authentication}.
